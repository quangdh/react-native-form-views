...
class SelectionInputDialog extends Component {
    constructor(props) {
        ...
        if (props.value && props.data && length(props.data) > 0) {
            if (props.keyExtractor) {
                let index = findIndex(item => JSON.stringify({ value: item }) === JSON.stringify({ value: props.value }), props.data);
                _value = props.keyExtractor(props.value, index);
            } else
                _value = props.value["id"];
        }
        this.state = {
    ...
    _renderItem(value, i) {
        ...
        return (
            <Wheel.Item label={label} value={key} key={key} />
        );
    }

    _renderHeader() {
        const { HeaderComponent, onBackdropPress, headerStyle, buttonCloseStyle, buttonCloseTextStyle, buttonCloseText } = this.props;
        if (HeaderComponent) return <HeaderComponent />
        else {
            return (
                <View style={[styles.header, headerStyle]}>
                    <TouchableOpacity onPress={onBackdropPress} style={buttonCloseStyle}>
                        <Text style={[styles.buttonClose, buttonCloseTextStyle]}>{buttonCloseText ? buttonCloseText : "X"}</Text>
                    </TouchableOpacity>
                </View>
            )
        }
    ...
    _renderFooter() {
        const { FooterComponent, buttonOKStyle, textButtonOK, buttonOKTextStyle } = this.props;
        if (FooterComponent) return <FooterComponent />
        else {
            return (
                <View style={styles.footer}>
                    <TouchableOpacity style={[styles.buttonOK, buttonOKStyle]} onPress={this._onPressOK}>
                        <Text style={[styles.textButtonOK, buttonOKTextStyle]}>{textButtonOK ? textButtonOK : "OK"}</Text>
                    </TouchableOpacity>
                </View>
            );
        }
