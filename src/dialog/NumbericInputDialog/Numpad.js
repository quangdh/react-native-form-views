...
class Numpad extends PureComponent {
    ...
    _renderItem(item) {
        ...
        if (item === "dot") {
            return renderButtonDot ? renderButtonDot(item) : (<Text style={[styles.button, buttonStyle]}>.</Text>)
        } else if (item === "back") {
            return renderButtonBack ? renderButtonBack(item) : (<Text style={[styles.button, buttonStyle]}>Back</Text>)
        } else {
    ...
    _renderRow() {
        ...
        this.NUM_PAD.forEach((item, index) => {
            rows.push(<View key={toString(index)} style={styles.row}>{this._renderItems(item)}</View>)
        });
        return rows;
    }
